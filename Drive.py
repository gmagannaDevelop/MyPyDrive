...
class Drive(object):
    ...
    def __init__(
        ...
        try:
            self.__gauth.LoadCredentialsFile(
                os.path.join(path_to_creds, credentials_file)
            )
        except Exception:
            pass
    ...
    def ez_query(
        ...
        _query = {"q": f"'{folder_id}' in parents {secondary} {tertiary}"}
    ...
    def upload_simple(
        ...
        try:
            if verbose:
                print(f"Uploading `{source}` as `{title}` ...", end="\t")
    ...
    def download_directory(
        ...
        if safe:  # Prevent trying to download files whose mimeType won't allow it.
            file_list = list(
                filter(lambda x: False if "vnd" in x["mimeType"] else x, file_list)
            )
